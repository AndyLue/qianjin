package com.dream.qianjin.easemob;
...
import com.dream.qianjin.EasemoUtil.HttpClient.HTTPClientUtils;
 ...
 * 
 */
public class EasemobUserUtil {

	private static Logger LOGGER = LoggerFactory.getLogger(EasemobUserUtil.class);
     ...
     * 注册IM用户[单个]
     */
    public static ObjectNode createUser(String userName,String passWord){
...
//        }
	...
	public static ObjectNode createNewIMUserBatchGen(String usernamePrefix, Long perNumber, Long totalNumber) {
		...
		ArrayNode genericArrayNode = EasemobUserUtil.genericArrayNode(usernamePrefix, totalNumber);
		if (totalNumber <= perNumber) {
			objectNode = EasemobUserUtil.createNewIMUserBatch(genericArrayNode);
		} else {
			...
			for (int i = 0; i < genericArrayNode.size(); i++) {
				...
				if ((i + 1) % perNumber == 0) {
					objectNode = EasemobUserUtil.createNewIMUserBatch(tmpArrayNode);
					if(objectNode!=null){
